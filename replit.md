...
- **Message Deduplication**: Sequence-based ordering system (inspired by WhatsApp/Signal/Kafka) prevents duplicate audio playback on mobile reconnections. Server assigns monotonic sequence numbers to each translation, maintains a 100-message buffer per room, and implements catch-up logic to send only missed messages (seq > lastReceivedSeq) when clients reconnect. Client tracks lastReceivedSeq in localStorage and includes it in join messages. Legacy messageId-based Set deduplication remains as defense-in-depth.
