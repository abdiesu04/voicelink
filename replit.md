...
- **Message Deduplication**: Multi-layered system prevents duplicate audio playback across all scenarios:
  ...
  - **Azure rescoring protection** (Tier 1): Language-agnostic fuzzy matching blocks duplicates when Azure Speech SDK double-fires the same utterance with different translations (e.g., "16 people" vs "41016 people"). Uses 95% original-text similarity threshold within 2-second window to catch Azure's rescoring behavior across all 47+ languages.
  - **Standard fuzzy matching** (Tier 2): Blocks duplicates where both original and translated texts are 82%+ similar, catching minor transcription variations.
