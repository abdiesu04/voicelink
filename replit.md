...
- **Audio Overlap Prevention**: Implemented a queue-based TTS system with synchronous boolean flag locking that GUARANTEES only one voice plays at a time in both directions. All translations are added to a FIFO queue and processed sequentially by a single processor. Critical features include: (1) **Synchronous boolean flag** (`isProcessingTTSRef`) prevents race conditions through atomic check-and-set before any await, (2) **Hybrid playback detection** using SpeakerAudioDestination's `onAudioEnd` event as primary mechanism with calculated duration timeout as fallback (whichever fires first), ensuring reliability across browsers, (3) Explicitly stopping ANY currently playing audio (both HTML5 Audio and Azure Synthesizer) before starting new audio, (4) Clean cleanup on component unmount with hasResolved flag to prevent double-triggering. The hybrid approach provides the accuracy of event-based detection when supported, with guaranteed timeout fallback for browser compatibility.
