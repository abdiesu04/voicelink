...
- **Audio Overlap Prevention**: Implemented a queue-based TTS system with Promise-based locking that GUARANTEES only one voice plays at a time in both directions. All translations are added to a FIFO queue and processed sequentially by a single processor. Critical features include: (1) **Promise-based lock** (`queueProcessorLockRef`) prevents race conditions - if multiple calls attempt to start processing, they wait for the current processor to finish, (2) Explicitly stopping ANY currently playing audio (both HTML5 Audio and Azure Synthesizer) before starting new audio, (3) Calculating audio duration from Azure's audio data bytes using: `byteLength / (16000 Hz × 2 bytes × 1 channel)` with 1000ms buffer, (4) Conservative fallback using 100 words/min + 1500ms buffer if audio data unavailable (min 2000ms wait), (5) Clean cleanup on component unmount. This eliminates the race condition where multiple `processTTSQueue` calls could create concurrent loops before the processing flag was set.
